    ...
    public class TarInputStream : Stream
    {
        ...
        public override int Read(byte[] inBuffer, int offset, int count)
        {
            ...
            if (this.readBuffer != null)
            {
                ...
                Array.Copy(this.readBuffer, 0, inBuffer, offset, sz);
            ...
            while (numToRead > 0)
            {
                ...
                if (recLen > sz)
                {
                    Array.Copy(rec, 0, inBuffer, offset, sz);
                ...
                {
                    sz = recLen;
                    Array.Copy(rec, 0, inBuffer, offset, recLen);
                }
        ...
        public TarEntry GetNextEntry()
        {
            ...
            else if (TarBuffer.IsEOFBlock(headerBuf))
            {
                this.hasHitEOF = true;
            }
            ...
            else
            {
                ...
                catch (InvalidHeaderException ex)
                {
                    ...
                    string errorText = string.Format("Bad header in record {0} block {1} {2}",
                                                     this.buffer.CurrentRecord, this.buffer.CurrentBlock, ex.Message);
