...
namespace Core.Logging.Log.SharpZipLib.Tar
{
    /// <summary>
    ...
    public class TarEntry : ICloneable
    {
        ...
        public TarEntry(ICloneable header)
        {
            if (header == null)
            {
        ...
        public bool IsDirectory
        {
            get
            {
                ...
                if (this.header != null)
                {
                    if ((this.header.TypeFlag == TarHeader.LF_DIR) || this.Name.EndsWith("/"))
                    {
                        return true;
                    }
                }
        ...
        /// </param>
        public void GetFileTarHeader(TarHeader inHeader, string inFile)
        {
            if (inHeader == null)
            {
                throw new ArgumentNullException( "inHeader" );
            }
...
*/
            ...
            inHeader.LinkName = String.Empty;
            inHeader.Name = name;

            if (Directory.Exists(inFile))
            {
                inHeader.Mode = 1003; // Magic number for security access for a UNIX filesystem
                inHeader.TypeFlag = TarHeader.LF_DIR;
                if ((inHeader.Name.Length == 0) || inHeader.Name[inHeader.Name.Length - 1] != '/')
                {
                    inHeader.Name = inHeader.Name + "/";
                }
            ...
            {
                inHeader.Mode = 33216; // Magic number for security access for a UNIX filesystem
                inHeader.TypeFlag = TarHeader.LF_NORMAL;
                inHeader.Size = new FileInfo(inFile.Replace('/', Path.DirectorySeparatorChar)).Length;
            }

            inHeader.ModTime =
                System.IO.File.GetLastWriteTime(inFile.Replace('/', Path.DirectorySeparatorChar)).ToUniversalTime();
            inHeader.DevMajor = 0;
            inHeader.DevMinor = 0;
        }
