...
namespace Core.Logging.Log.SharpZipLib.Zip
{
    ...
    public class ZipEntry : ICloneable
    {
        ...
        public bool HasCrc
        {
            get { return (this.known & Known.Crc) != 0; }
        }
        ...
        {
            get { return (this.flags & (int) GeneralBitFlags.UnicodeText) != 0; }
            set
            {
                if (value)
                {
        ...
        {
            get { return (this.known & Known.ExternalAttributes) == 0 ? -1 : this.externalFileAttributes; }
        ...
        {
            get { return (this.versionMadeBy & 0xff); }
        }
        ...
        {
            get { return (this.versionMadeBy >> 8) & 0xff; }
        ...
        {
            get { return this.LocalHeaderRequiresZip64 || (this.offset >= uint.MaxValue); }
        }
        ...
        public DateTime DateTime
        {
            get
            {
                ...
                uint sec = 2*(this.dosTime & 0x1f);
                uint min = (this.dosTime >> 5) & 0x3f;
                uint hrs = (this.dosTime >> 11) & 0x1f;
                uint day = (this.dosTime >> 16) & 0x1f;
                uint mon = ((this.dosTime >> 21) & 0xf);
                uint year = ((this.dosTime >> 25) & 0x7f) + 1980;
                return new DateTime((int) year, (int) mon, (int) day, (int) hrs, (int) min, (int) sec);
            }
            set
            {
                this.DosTime = ((uint) value.Year - 1980 & 0x7f) << 25 |
                               ((uint) value.Month) << 21 |
                               ((uint) value.Day) << 16 |
                               ((uint) value.Hour) << 11 |
                               ((uint) value.Minute) << 5 |
        ...
        public long Size
        {
            get { return (this.known & Known.Size) != 0 ? (long) this.size : -1L; }
            set
            {
        ...
        {
            get { return (this.known & Known.CompressedSize) != 0 ? (long) this.compressedSize : -1L; }
            set
            {
        ...
        public long Crc
        {
            get { return (this.known & Known.Crc) != 0 ? this.crc & 0xffffffffL : -1L; }
            set
            {
        ...
        /// </returns>
        /// <see cref="Core.Logging.Log.SharpZipLib.Zip.CompressionMethod.Deflated"/>
        /// <see cref="Core.Logging.Log.SharpZipLib.Zip.CompressionMethod.Stored"/>
        ...
        public void SetExtraData( byte[] inBytes )
        {
            if( inBytes == null )
            {
                this.extra = null;
            ...
            {
                if( inBytes.Length > 0xffff )
                {
                    throw new ArgumentOutOfRangeException( "inBytes" );
                }

                this.extra = new byte[inBytes.Length];
                Array.Copy( inBytes, 0, this.extra, 0, inBytes.Length );
            }
        }
        ...
        public bool IsFile
        {
            get { return !this.IsDirectory && !this.HasDosAttributes(8); }
        }
...
*/
            if (extraData.Find(0x5455))
            {
                ...
                int lFlags = extraData.ReadByte();

                // Can include other times but these are ignored.  Length of data should
                // actually be 1 + 4 * no of bits in flags.
                if (((lFlags & 1) != 0) && (length >= 5))
                {
