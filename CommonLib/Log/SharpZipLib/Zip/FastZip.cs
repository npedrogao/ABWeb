...
namespace CommonLib.Log.SharpZipLib.Zip
{
    ...
    public class FastZip
    {
        ...
        public IEntryFactory EntryFactory
        {
            ...
            set { this.entryFactory_ = value ?? new ZipEntryFactory(); }
        }
        ...
        {
            get { return this.restoreDateTimeOnExtract_; }
                               ...
                               string fileFilter, string directoryFilter, bool restoreDateTime)
        {
            ...
            using (this.zipFile_ = new ZipFile(zipFileName))
            {
                ...
                while (this.continueRunning_ && enumerator.MoveNext())
                {
                    ...
                    if (entry.IsFile)
                    {
                        if (this.directoryFilter_.IsMatch(Path.GetDirectoryName(entry.Name)) &&
                            this.fileFilter_.IsMatch(entry.Name))
                        {
        ...
        private void ExtractEntry(ZipEntry entry)
        {
            ...
            if (doExtraction)
            {
                ...
                {
                    targetName = Path.Combine(this.targetDirectory_, nameText);
